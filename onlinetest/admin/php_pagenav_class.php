...
class buildNav {
      ...
      function show_num_pages($frew = "&laquo;", $rew = '上一页', $ffwd = '&raquo;', $fwd = '下一页', $separator = '') {
               ...
               if($this->num_pages > 1) {
                  ...
                  if (!empty($rew)) {                                                                                                                                                                                                                                              //$separator [$frew] $rew
                      $return .= ($_GET[$this->offset] > 0) ? "<a href=\"$_GET[PHP_SELF]?$this->offset=0$this->_get_vars\" $this->theClass title=\"第一页\">$frew</a> <a href=\"$_GET[PHP_SELF]?$this->offset=$this->listPrev$this->_get_vars\" $this->theClass title=\"上一页\">$rew</a> $separator " : "";
                  }
                  ...
                  if ($this->show_pages_number || !isset($this->show_pages_number)) {
                      ...
                      if (!empty($fwd)) {
                          ...
                          $return .= ($_GET[$this->offset] + $this->limit < $this->total_result) ? "$separator <a href=\"$_GET[PHP_SELF]?$this->offset=$this->listNext$this->_get_vars\" $this->theClass title=\"下一页\">$fwd</a> <a href=\"$_GET[PHP_SELF]?$this->offset=$offset_end$this->_get_vars\" $this->theClass title=\"最后一页\">$ffwd</a>" : "";
                      }
                  }
               }
               return $return;
      ...
      function show_info() {
               $return .= "共: ".$this->total_result." 记录, ".$this->num_pages."页 ";
